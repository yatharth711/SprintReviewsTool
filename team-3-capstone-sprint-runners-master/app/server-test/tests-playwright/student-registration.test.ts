...
async function mockAPIResponses(page: any) {
  await page.route('**/api/addNew/addStudent', (route: { fulfill: (arg0: { status: number; body: string; }) => void; }) => {
    route.fulfill({
      status: 200,
...
test.describe('Student Registration Page', () => {
  ...
  test.beforeEach(async ({ page }) => {
    // Navigate to the student registration page before each test
    await page.goto(`${baseURL}/student/registration`);
  });

  // Check that the registration page loads correctly
  test('should load registration page', async ({ page }) => {
    await expect(page).toHaveURL(`${baseURL}/student/registration`);
  ...
  test('should show validation errors for empty fields', async ({ page }) => {
    ...
    await expect(page.locator('text=Student ID cannot be empty')).toBeVisible();
  ...
  test('should register successfully', async ({ page }) => {
    ...
    await page.fill('input[aria-label="Student ID"]', '12345');
    ...
    await expect(page).toHaveURL(`${baseURL}/student/login`);
